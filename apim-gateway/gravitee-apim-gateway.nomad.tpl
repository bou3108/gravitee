job "gravitee-apim-gateway" {
    ...
    group "apim-gateway" {
	    ...
	    artifact {
	    	source	= "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/asip-snapshots/fr/ans/psc/generateVIHF/1.5.1-SNAPSHOT/generateVIHF-1.5.1-SNAPSHOT.zip"
		options {
			archive = false
		}
	    }
		...
		mount {
			type = "bind"
			target = "/opt/graviteeio-gateway/plugins/generateVIHF-1.5.1-SNAPSHOT.zip"
			source = "local/generateVIHF-1.5.1-SNAPSHOT.zip"
			readonly = false
			bind_options {
				propagation = "rshared"
				}
